  ...
  target=$(
    {
      ...
      | while IFS="|" read -r date_secs branch relative subject; do
          ...
          reset="\033[0m"
          printf "%s|${color}%s${reset}|%s|local\n" "$branch" "$relative" "$subject"
        done
      
      # Remote branches (only recent ones without local counterparts)
      git for-each-ref --sort=-committerdate --format="%(committerdate:unix)|%(refname:short)|%(committerdate:relative)|%(subject)" refs/remotes/ \
      | while IFS="|" read -r date_secs branch relative subject; do
          ...
          reset="\033[0m"
          printf "%s|${color}%s${reset}|%s|remote\n" "$branch" "$relative" "$subject"
        done
    } \
    | column -t -s '|' \
